...
def build_recipe_nutrition_map(recipes) -> Dict[int, NutritionInfo]:
    """批量计算食谱营养，避免同一请求内重复计算"""
    return {
...
def recipe_nutrition_summary(recipe: Recipe, nutrition: NutritionInfo = None) -> Dict[str, Any]:
    """构建图表和列表共用的食谱营养摘要"""
...
def index():
    ...
    # 热量柱状图（全部食谱，供前端下拉切换）
    all_recipes_for_bar = [
        recipe_nutrition_summary(r, nutrition_map[id(r)])
        for r in recipes
    ]
    # 默认取热量最高的8道作为初始柱状图
    top_recipes = sorted(all_recipes_for_bar, key=lambda r: r['calories'], reverse=True)[:8]
    ...
    first_nutrition = nutrition_map[id(recipes[0])] if recipes else None
    radar_data = None
    if first_nutrition:
        radar_data = {
        ...
        'recipes': [
            {
                ...
                'values': [
                    ...
                    round(min(r['fiber'] / 10 * 100, 100), 1),
                    round(min(r['vitamin_a'] / 900 * 100, 100), 1),
                    round(min(r['vitamin_c'] / 90 * 100, 100), 1),
                ],
            }
...
def api_charts():
    ...
    top_recipes = sorted(all_for_bar, key=lambda r: r['calories'], reverse=True)[:8]
    ...
    # 营养雷达图数据（第一个食谱）
    if recipes:
        first_nutrition = nutrition_map[id(recipes[0])]
        radar_data = {
            'recipe': recipes[0].name,
            'labels': ['蛋白质', '脂肪', '碳水', '纤维', '维生素A', '维生素C'],
            'values': [
